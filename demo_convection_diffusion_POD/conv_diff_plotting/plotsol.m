...
uu=zeros(ns,nt);
for trinode=1:dim+1
    du=ss(:,trinode)*(u(t(:,trinode),1))';    
    uu=uu+du;
end

pvis=reshape(permute(pp,[1,3,2]),[ns*nt,2]);
tvis=kron(ones(nt,1),tt)+kron(ns*(0:nt-1)',0*tt+1);

cla,patch('vertices',pvis,'faces',tvis,'cdata',uu, ...
